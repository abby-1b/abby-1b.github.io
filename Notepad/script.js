...
let math = {
	bin: (n) => { return parseInt(n + "", 2) },
	log: (n) => { console.log(n) },
	red: function reduce(numerator, denominator){
		const gcd = function gcd(a,b) { return b ? gcd(b, a % b) : a }
...
function drawGraph(g, v, context, id) {
	...
	try {
		...
		for (let x = -cv.width / 2 - graphStep; x < cv.width / 2 + graphStep; x += graphStep) {
			let y = function(){
				return eval("with(storeVars(this)){" + g.replace(new RegExp(v, 'g'), x / graphScale) + "}")
			}.call(context) * graphScale
			cvc.moveTo(x, -y)
...
function updateInfo() {
	document.getElementById('if').value = tabMenu
}
...
function mathAnalyze() {
	...
	for (let s = 0; s < ov.children.length; s++) {
		...
		returned = "!!!"
		if (str[s][str[s].length - 1] == '\\' || str[s].endsWith('= ')) {
			// Math: Implements multiplication without an operator.
			ctx = mathProcess(str[s].slice(0, -2), ctx)
			ov.children[s].innerHTML = "&nbsp;".repeat(str[s].length) + returned
