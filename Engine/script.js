...
let player = con.addSprite(Sprite("Sprites/Player.png", con.width / 2, con.height / 2, 16, 16, true))
player.speed = new Vec2(0, 0)
player.animationFrames = 6
player.animationTimer = 6
player.hbOffsets({top: 0, bottom: 0, left: 6, right: 6})

let floor = con.addSprite(Sprite("Tiles/SmallBrick.png", con.width / 2, con.height / 2, 16, 16, true))
...
con.loop(() => {
    ...
    if (' ' in con.keys && player.onGround) {
        player.animationFrame = 0
    ...
    if (player.yp > con.height / 2) {
        player.speed.y = 0
        player.yp = con.height / 2
    ...
    console.log(player.intersects(floor))
